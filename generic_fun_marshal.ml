...
let visit_val v = if Obj.is_int v then v else H.find visit_val_table v
...
let check_size s v =
  exn "Incorrect block size" (lazy (guard (Obj.is_block v && Obj.size v = s)))
let check_tag t v =
  exn "Incorrect tag" (lazy (guard (Obj.is_block v && Obj.tag v == t)))

let is_custom s v =
  Obj.tag v == Obj.custom_tag
 ...
 *)
let obj_to_repr r x = Obj.repr (r.Repr.to_repr (Obj.obj x))
let obj_from_repr r x =
  match r.Repr.from_repr (Obj.obj x) with
  | Some y -> Obj.repr y
  | None -> raise (Serialize_exception "Invalid abstract representation.")

let obj_update r x y = r.Repr.update (Obj.obj x) (Obj.obj y)
let obj_default r = Obj.repr r.Repr.default

(* Set the field in each back link to the new (abstract) value *)
let restore_edge a _
  = let a' = visit_val a
    in List.iter (fun (b,i) ->
           Obj.set_field (visit_val b) i a')
...
and check : type a . a ty -> obj -> obj
  = fun t v ->
(* Format.printf "MARSHAL CHECK: %s@." (Obj_inspect.show_obj t); *)
  let t = Antiunify.resolve_synonym t in
  if Obj.is_block v && not (visit_val_mem v) then
    visit_val_set v (Obj.dup v);
  if Obj.is_int v     (* immediate values can have many types *)
      ...
      | Abstract ->
        (match try Repr.repr t
           ...
           | Repr.Repr r ->
             match !direction with
             | To ->
               let w = Obj.new_block 0 1 in (* tag 0, size 1 *)
               if Obj.is_block v then visit_val_set v w;
               Obj.set_field w 0 (check r.repr_ty (obj_to_repr r v));
               w
             | From ->
               visit_val_set v (obj_default r);
               let f1 = Obj.field v 0 in (* no back link for this indirection *)
...
and check_concrete : type a . a ty -> obj -> unit
  = let open Ty.T in fun ty ->
(* Format.printf "CHECKING TYPE: %s@." *)
(*    (Obj_inspect.show_obj ty); *)
match ty with
  (*| Var n       -> raise (Serialize_exception "Inhabited polymorphic type (Var)")  I removed variables*)
  | Ty.Any         ->
 fun o ->
(* print_endline "ANY TYPE DETECTED"; *)
   (* Obj_inspect.print_obj o; *)
   raise (Serialize_exception "Inhabited polymorphic type.")
  | Int         -> guard -< Obj.is_int
  | Char        -> guard -< is_char
  | Float       -> check_tag Obj.double_tag >>. check_size 1
  | Bytes       -> check_tag Obj.string_tag
  | String      -> check_tag Obj.string_tag
  | Array Float -> check_tag Obj.double_array_tag
...
and check_field : type a . a ty -> obj -> int -> unit
  = fun t v i ->
    with_field v i (fun x -> Obj.set_field (visit_val v) i (check t x))
...
and check_record_fields : type p r . int -> (p, r) Desc.Fields.t -> (p -> r) -> obj -> unit
  = fun i r fwd v ->
    ...
    guard (Objx.fields_all2 (==) v (Obj.repr (fwd (Obj.obj (Objx.listify v)))))
...
and check_variant : type v . v Desc.Variant.t -> obj -> unit
  = fun d v ->
    ...
    else if Obj.is_int v
         && Fun.in_range (Obj.obj v) (0, Desc.Variant.cst_len cs - 1)
    then ()
    else begin
      guard (Obj.tag v < Desc.Variant.ncst_len cs);
      check_args (Desc.Variant.ncst_get cs (Obj.tag v)) v;
    end
...
and check_poly_variant : type v . v Desc.Poly.t -> obj -> unit
  ...
  try let k = Objx.poly_hash v in
      ...
      one_of [ lazy (guard (Obj.is_int v && Desc.Con.arity c == 0))
             ; lazy begin
...
and check_extensible : type t . t Desc.Ext.t -> obj -> unit
  = fun x v ->
  try let w = Desc.Ext.fix x (Obj.obj v) in
      visit_val_set v (Obj.repr w);
      let Desc.Con.Con c = Desc.Ext.con x w
      in one_of [ lazy (guard (Obj.tag v == Obj.object_tag)) (* constant constructor *)
                ; lazy (match c.args with
                      | Product p -> check_fields_from 1 p (Obj.repr w)
                      | Record r -> check_record_fields 1 r c.embed (Obj.repr w))]
...
and check_lazy : type t . t ty -> obj -> obj
  = fun t v ->
    match Objx.tag_view (Obj.tag v) with
...
let to_repr : 'a ty -> 'a -> obj
  = fun t x ->
    convert To t (Obj.repr x)

(* Raise: Serialize_exception (TODO: document other exceptions that may be raised) *)
let from_repr : 'a ty -> obj -> 'a
  = fun t v ->
    Obj.obj (convert From t v)
...
let safe_cast : 'a ty -> obj -> 'a
  = fun t v ->
    if Obj.is_block v && Obj.size v == 2 then
      let ty = Generic_fun.Deepfix.deepfix (Ty t) (Obj.obj (Obj.field v 0))
      and rep = Obj.field v 1
